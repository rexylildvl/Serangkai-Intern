...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center mb-8">
        ...
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Edit Galeri Foto</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi foto galeri</p>
        </div>
    </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 sm:p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#7aa06e] focus:border-[#7aa06e] transition duration-200"
                          placeholder="Masukkan deskripsi foto">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative w-full max-w-xs">
                        <img src="{{ asset('storage/' . $galeri->foto) }}" 
                             alt="Current Photo" 
                             class="w-full h-48 object-cover rounded-lg shadow-md border">
                    </div>
                </div>
                
                {{-- New Photo Upload --}}
                <label class="block text-sm font-medium text-gray-700 mb-2">Unggah Foto Baru (biarkan kosong jika tidak ingin mengubah)</label>
                ...
                <div id="preview-container" class="mt-4 hidden">
                    <p class="text-sm text-gray-500 mb-2">Pratinjau foto baru:</p>
                    <div class="relative">
                        <img id="preview-foto" class="w-full h-48 object-cover rounded-lg shadow-md border">
