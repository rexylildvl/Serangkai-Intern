...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Daftar lengkap berita yang telah dipublikasikan</p>
        </div>
    ...
    @if($news->isEmpty())
        <div class="bg-white rounded-lg shadow-sm p-6 text-center">
            ...
            <h3 class="mt-2 text-lg font-medium text-gray-900">Belum ada berita</h3>
            <p class="mt-1 text-sm text-gray-500">Mulai dengan menambahkan berita baru</p>
            <div class="mt-6">
                <a href="{{ route('admin.berita.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 transition">
                    Tambah Berita
                </a>
            </div>
        </div>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($news as $item)
                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                    <div class="relative">
                        <img src="{{ asset('storage/' . $item->foto) }}" alt="Foto Berita" class="w-full h-48 object-cover">
                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                            <h3 class="text-lg font-semibold text-white">{{ $item->judul }}</h3>
                        </div>
                    </div>
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex justify-between items-center border-t border-gray-100 pt-3">
                            <a href="{{ route('admin.berita.show', $item->id) }}" 
                               class="text-green-600 hover:text-green-900 text-sm font-medium">Detail</a>
        ...
        {{-- Menampilkan pagination hanya jika metode links() tersedia --}}
